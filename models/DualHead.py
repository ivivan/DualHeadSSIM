...
class Shared_Encoder(nn.Module):
    def __init__(self, input_dim, enc_hid_dim, dec_hid_dim, enc_layers,
                 dec_layers, dropout_p, batch_size):
    ...
    def forward(self, input_before, input_after, hidden_before, hidden_after):
        ...
        outputs_after, hidden_after = self.gru(embedded_after, hidden_after)
    ...
    def initHidden(self):
        return torch.zeros(self.enc_layers * 2, self.batch_size,
                           self.enc_hid_dim)
...
class DualSSIM(nn.Module):
    ...
    def forward(self, src_left, src_right, trg, teacher_forcing_ratio=0.5):
        ...
        # Shared Encoder
        hidden_before = self.shared_encoder.initHidden()
        hidden_after = self.shared_encoder.initHidden()

        encoder_outputs_left, encoder_outputs_right, hidden = self.shared_encoder(
            src_left, src_right, hidden_before, hidden_after)
